...
function init() {
	createVis1(".vis1");
	createVis2(".vis2");
	//Vis3 is created by the initiallization of the buttons
...
function RadarChart(parent_selector, data, options) {
	...
	blobCircleWrapper
		...
		.on("mouseout", function () {tooltip.transition().style("display", "none").text("");});
...
function updateNet() {
	...
	if(!(net_relation == "" || net_relation.startsWith("None") || net_relation.endsWith("None"))){
		if (centre == "") {
...
function changeRadio(radio_selection) {
	...
	if (radio_group_name == relation_name || 
		...
		((radio_group_name == "class" || radio_group_name == "race") && relation_name == "combo")
	) {
		updateNet();
		updateRadar();
	}
