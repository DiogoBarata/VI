...
with open(filename + ".json","r",encoding="utf-8") as f:
    ...
    for character in data:
        ...
        for char_class in data[character]['class']:
            originality = 1/(race_count[current_race] * class_count[char_class])
            tmp = {'originality': originality}
            (data[character]['class'][char_class]).update(tmp)
            if originality < min_org:
                min_org = originality
            if originality > max_org:
                max_org = originality

    skills_count = {}
    for character in data:
        skills = data[character]['feats']
        for skill in skills:
